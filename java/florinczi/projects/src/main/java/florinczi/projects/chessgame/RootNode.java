...
public class RootNode {
    ...
    public Board minmaxRoot(){
        ...
        System.out.printf("Chosen this move with expected eval %.2f%n. Analysed %d nodes%n", eval,nodeCount);
    ...
    private float minmax (Board board, int depth){
        ...
        if (depth == 0 || board.getMoveList().isEmpty()){
            ...
            for (Board b: boardList){
                ...
                System.out.printf("Currently depth %d, branch %d out of %d%n", depth, iter++, boardList.size());
                Menu.printBoard(board);
